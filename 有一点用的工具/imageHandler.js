 ...
 * @LastEditTime: 2025-02-15 20:41:17
 * @Description:
...
class ImageHandler {
  ...
  handleMouseMove(e) {
    ...
    // 应用边界限制
    this.config.offsetX = Math.min(
      maxOffsetX,
      Math.max(minOffsetX, newOffsetX)
    );
    this.config.offsetY = Math.min(
      maxOffsetY,
      Math.max(minOffsetY, newOffsetY)
    );
  ...
  handleWheel(e) {
    ...
    const newScale = Math.max(
      minScale,
      Math.min(maxScale, this.config.scale * delta)
    );
  ...
  getImageBounds() {
    return {
      width: this.image ? this.image.width : this.canvas.width,
      height: this.image ? this.image.height : this.canvas.height,
    };
  }
}
